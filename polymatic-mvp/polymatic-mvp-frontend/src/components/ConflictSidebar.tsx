...
import { AlertTriangle, ExternalLink, RefreshCw, MapPin } from 'lucide-react';
...
export default function ConflictSidebar() {
  const { layers, updateEntities, setActivePoi } = useStore();
  ...
  }, [layers.conflicts, updateEntities]);
    ...
    <AnimatePresence>
      {layers.conflicts && (
        <motion.div 
          ...
          className="absolute top-0 left-80 h-full w-96 bg-zinc-950/95 border-r border-zinc-800 text-zinc-300 p-4 z-10 flex flex-col font-mono text-sm backdrop-blur-md shadow-2xl overflow-hidden"
        >
